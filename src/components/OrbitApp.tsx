...
export default function OrbitApp({
  ...
  useEffect(() => {
    ...
    const storedLabels = localStorage.getItem("orbit_labels");
    if (storedLabels) setCustomLabels(JSON.parse(storedLabels));
  }, []);

  const labels: Record<string, string> = { ...RELATIONSHIP_LABELS, ...customLabels };

  function updateLabel(relationship: string, label: string) {
    const updated = { ...customLabels, [relationship]: label };
    setCustomLabels(updated);
    localStorage.setItem("orbit_labels", JSON.stringify(updated));
  }
  ...
  const dueSoon = useMemo(
    () => contacts.filter(
      (c) => nudgeFor(c.lastContactedAt, c.relationship, nudgeWindows).status === "due",
    ),
    [contacts, nudgeWindows],
  );
  ...
  return (
    <div className="flex min-h-screen flex-col">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-500">Keep your network warm</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="btn-ghost text-xs" onClick={() => setShowProfile((s) => !s)}>
              {showProfile ? "Hide settings" : "Settings"}
            </button>
      ...
      {/* Overview bar */}
      <div className="border-b border-white/[0.04] bg-orbit-surface/40">
        <div className="mx-auto flex max-w-7xl items-center gap-6 px-5 py-2">
          <OverviewStat label="Total" value={contacts.length} />
          <div className="h-3 w-px bg-white/[0.08]" />
          <OverviewStat
            label="Overdue"
            value={overdue.length}
            tone={overdue.length > 0 ? "red" : "neutral"}
            onClick={overdue.length > 0 ? () => setSelectedId(overdue[0].id) : undefined}
          />
          <OverviewStat
            label="Due soon"
            value={dueSoon.length}
            tone={dueSoon.length > 0 ? "yellow" : "neutral"}
            onClick={dueSoon.length > 0 ? () => setSelectedId(dueSoon[0].id) : undefined}
          />
          <OverviewStat
            label="Fresh"
            value={contacts.length - overdue.length - dueSoon.length}
            tone="neutral"
          />
        </div>
      </div>
        ...
        <section className="min-w-0 flex-1">
          {selected ? (
            <ContactDetail
              ...
              onLogInteraction={(kind, note) => logInteraction(selected.id, kind, note)}
              onDeleteInteraction={(interactionId) => deleteInteraction(selected.id, interactionId)}
...
function OverviewStat({
  ...
  const valueColor =
    tone === "red" ? "text-orange-400" :
    tone === "yellow" ? "text-amber-400" :
    "text-white";
  const Tag = onClick ? "button" : "div";
  return (
    <Tag
      className={`flex items-baseline gap-1.5 ${onClick ? "cursor-pointer hover:opacity-80 transition-opacity" : ""}`}
      {...(onClick ? { onClick } : {})}
    >
      <span className={`text-base font-semibold tabular-nums ${valueColor}`}>{value}</span>
